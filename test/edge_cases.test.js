...
QUIT;
  ...
  assert.ok(nodes.some(n => n.data.label === "A_B_C"), "Should handle underscores");
  assert.ok(nodes.some(n => n.data.label === "X123"), "Should handle numbers");
});
...
D_LABEL:
  ...
  const nodes = elements.filter(e => !e.data.source);
  assert.ok(nodes.some(n => n.data.label === "B"), "Should fork to B");
  assert.ok(nodes.some(n => n.data.label === "C"), "Should fork to C");
  assert.ok(nodes.some(n => n.data.label === "D"), "Should fork to D");
});
...
test("resolve with empty threads map", () => {
  ...
  assert.strictEqual(elements.length, 0, "Empty threads should produce no elements");
});

test("resolve with single thread", () => {
  const threads = new Map();
  threads.set("0", [
    { id: "1", label: "A", forks: [], joins: [] }
  ]);
